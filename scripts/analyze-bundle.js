...
if (!fs.existsSync(statsPath)) {
  console.log('Stats file not found. Run "npm run build" first to generate stats.');
  process.exit(1);
}
...
try {
  ...
  if (stats.chunks) {
    ...
    stats.chunks.forEach(chunk => {
      ...
      console.log(`  ${chunk.names ? chunk.names.join(', ') : chunk.id}: ${sizeKB} KB (${modules} modules)`);
    });
  }
  ...
  if (stats.modules) {
    ...
    stats.modules
      ...
      .forEach((module, index) => {
        ...
        const cleanName = name.replace(/^.*node_modules[\/\\]/, '').replace(/\?.*$/, '');
